...
Mbed Studio Setup
Mbed Studio is a free IDE for creating Mbed OS 6 applications and libraries. Download and install it from: https://os.mbed.com/studio/. Register for an account and sign in when launching the IDE.
...
Open File > New Program.
Select “Empty Mbed OS program” to start from scratch or choose a pre-written example.
Name the program in the “Program Name” field and click Add Program (ensure “make this the active program” is ticked).

To import the “mbed-os-example-blinky” project:

Open File > New Program.
Select “mbed-os-example-blinky” from the dropdown and click Add Program.

The program will appear in the explorer on the left-hand side of the IDE.
Building and Running a Program

Connect your Mbed-enabled board and select it from the target list in Mbed Studio.
Ensure “mbed-os-example-blinky” is the active program, then click Build program.
Mbed Studio creates a .bin file (compiled machine code). Copy this file to the board’s internal storage to load and run the program (e.g., blinking LED).
Alternatively, click Run program to automatically build and copy the program to the board.
...
Getting Set-up
Include the board support files for the DISCO-L475VG-IOT01A from: https://os.mbed.com/teams/ST/code/BSP_B-L475E-IOT01/. In Mbed Studio, go to File > Add Library to Active Program, paste the URL, and select the default/master branch.
...
Include the BLE utility library: https://github.com/ARMmbed/mbed-os-ble-utils
Include the board support library as in Lab 2.

Working with Bluetooth
Refer to:

Mbed OS Bluetooth APIs: https://os.mbed.com/docs/mbed-os/v6.10/apis/bluetooth-apis.html
BLE examples: https://github.com/ARMmbed/mbed-os-example-ble/
...
DISCO-L475VG-IOT01A board
Mbed Studio or another suitable development environment
Google Cloud account: https://cloud.google.com (additional charges may apply if usage limits are exceeded)

Getting Started
The Development Board
Create a blank Mbed OS 6 project and add:

Board Support files: https://os.mbed.com/teams/ST/code/BSP_B-L475E-IOT01/
ISM43362 Wi-Fi component: https://github.com/ARMmbed/wifi-ism43362/
Mbed OS 6 HTTP client: https://github.com/rasmus0201/mbed-http-client.git
...
The Cloud
Create a Google Cloud project (e.g., “arm-edx-lab4”) in the Google Cloud Console: https://console.cloud.google.com. Note the Project ID.
...
Navigate to Credentials > Manage service accounts > Create service account.
Name it (e.g., “datastore”) and assign the “Cloud Datastore User” role.
Create a JSON key and save it securely.
...
function computeStdDev(array) {
    ...
    return Math.sqrt(array.reduce((a, b) => a + Math
